...
if (!function_exists('wpbucket_breadcrumbs')) {
    ...
    function wpbucket_breadcrumbs()
    {
        ...
        $home = esc_html__('Home', 'yowel'); // text for the 'Home' link
        ...
        $home_html = "<li><a href='{$homeLink}' class='home'>{$home}</a>{$delimiter}</li>";
        ...
        if (is_front_page()) {
            ...
            if (is_category()) {
                ...
                echo '<li>' . $before . esc_html__('Category:', 'yowel') . ' "' . single_cat_title('', false) . '"' . $after . '</li>';
            } elseif (is_search()) {
                echo '<li>' . $before . esc_html__('Search results:', 'yowel') . ' "' . get_search_query() . '"' . $after . '</li>';
            } elseif (is_day()) {
                echo wp_kses_post($blog_page_link);
                ...
                if (get_post_type() != 'post') {
                    ...
                    if ($post_type->name == 'pi_portfolio') {
                        ...
                        echo '<li><a href="' . $homeLink . '' . $slug['slug'] . '/" title="' . $home . '">' . $title . '</a>' . $delimiter . '</li>';
                        if ($showCurrent == 1) {
                ...
                if (!empty($parent)) {
                    ...
                    echo '<li><a href="' . esc_url(get_permalink($parent)) . '">' . $parent_title . '</a>' . $delimiter . ' </li>';
                    if ($showCurrent == 1) {
                        $att_title = substr(get_the_title(), 0, $breadcrumbs_length);
                        echo ' <li>' . $before . $att_title . $after . '</li>';
                    }
                }
                ...
                if ($showCurrent == 1)
                    echo '<li>' . $before . $vlc_page_title . $after . '</li>';
            } elseif (is_page() && $post->post_parent) {
                ...
                if ($navigation_label == '1') {
                    ...
                    if ($showCurrent == 1 && !empty($menu)) {
                        ...
                        $title = wpbucket_substr(get_the_title(), $breadcrumbs_length, 3);
                        echo ' <li>' . $before . $title . $after . '' . $delimiter . ' </li>';
                    }
                } else {
                    if ($showCurrent == 1) {
                        $title = wpbucket_substr(get_the_title(), $breadcrumbs_length, 3);
                ...
                echo '<li>' . $before . esc_html__('Tag:', 'yowel') . ' "' . single_tag_title('', false) . '"' . $after . '</li>';
            } elseif (is_author()) {
                global $author;
                $userdata = get_userdata($author);
                echo wp_kses_post($blog_page_link);
                echo '<li>' . $before . esc_html__('Author: ', 'yowel') . esc_html($userdata->display_name) . $after . '</li>';
            } elseif (is_404()) {
                echo '<li>' . $before . esc_html__('Error 404', 'yowel') . $after . '</li>';
            }

            if (get_query_var('paged')) {
                echo '<li> ' . esc_html__('Page', 'yowel') . ' ' . get_query_var('paged') . '</li>';
            }
